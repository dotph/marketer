# Verified by dotPH — Campaign Brief

**Product:** Verified by dotPH — free identity verification for .ph domain holders
**Stage:** Pre-launch (product not yet live)
**Goal:** Collect signups. Build demand so businesses want and feel the need for verification.
**Timeline:** 8 weeks
**Budget:** $500–$2,000/month (Facebook Ads)

---

## The One Idea

**Lead with the problem, not the product.**

People don't want verification. They want their customers to trust them and stop paying scammers. The campaign makes them feel that problem first, then presents Verified by dotPH as the answer.

---

## 3-Phase Strategy

| Phase | Weeks | Message | Goal |
|-------|-------|---------|------|
| **1. Problem Awareness** | 1–3 | "This is happening to businesses like yours." | Create the need |
| **2. Solution Reveal** | 3–5 | "There's now a way to prove you're real." | Position dotPH as the answer |
| **3. Signup Drive** | 5–8 | "Get verified before everyone else does." | Convert interest to signups |

---

## Channels

| Channel | Audience | Role |
|---------|----------|------|
| **Email — Warm** | 10K-50K existing .ph holders | Lead the arc. 4-email sequence over 4 weeks. |
| **Email — Cold** | New prospects (non-.ph holders) | 5-email sequence over 18 days. Problem → solution → FOMO. |
| **Facebook Ads** | PH business owners 25-55 | Awareness, retargeting, conversion. 3 campaigns. |
| **Organic Facebook** | dotPH page audience | Problem education, product posts, social proof. 3x/week. |

All channels point to the landing page. The landing page converts.

---

## Campaign Arc by Channel

```
Week 1-2   Email 1 (problem seed)  |  FB Ads: Awareness  |  Organic: Problem posts
Week 3     Email 2 (introduce product)  |  FB Ads: Retarget  |  Organic: Product reveal
Week 4-5   Email 3 (social proof)  |  FB Ads: Conversion  |  Cold Emails 1-4
Week 6-8   Email 4 (final push)  |  FB Ads: Shift to conversion  |  Cold Email 5  |  Daily posts
```

---

## Key Messages

> "Your customers can't tell if your business page is real."

> "Prove you're the real business behind your .ph domain."

> "Free identity verification from the official .ph registry."

> "Don't let scammers profit from your brand."

---

## Facebook Ads — 3 Campaigns

**Campaign 1: AWARENESS** (50% of budget, Weeks 1-3)
- Audience: Business owners PH + industry-specific (hospitality, F&B, retail, healthcare)
- Creative: Impersonation stories, scam stats, "Is someone copying your business?"
- Objective: Reach / Video Views

**Campaign 2: CONSIDERATION** (30% of budget, Weeks 3+)
- Audience: Retarget engaged users from Campaign 1 + lookalikes
- Creative: "Verified by dotPH" explainer, how it works, badge showcase
- Objective: Traffic → Landing page views

**Campaign 3: CONVERSION** (20% → shift to 40% by Week 5)
- Audience: Retarget landing page visitors who didn't sign up
- Creative: Social proof count, "It's free. What are you waiting for?", objection busters
- Objective: Conversions → Signup

---

## Top Ad Angles (Facebook)

1. **Impersonation fear** — "A fake page has your name, your logo, your photos."
2. **Payment hesitation** — "Is this your real GCash?" — every. single. transaction.
3. **Blue check analogy** — Instagram has it. Twitter has it. Now PH businesses have one.
4. **Free + authority** — "Free. From the official .ph domain registry."
5. **Peer pressure** — "Your competitors are getting verified. Are you?"

---

## Email Strategy — Warm (.ph Holders)

| # | Send | Subject | Angle |
|---|------|---------|-------|
| 1 | Week 1 | Is someone pretending to be your business online? | Problem seed (no pitch) |
| 2 | Week 2 | Your domain proves ownership. But does it prove legitimacy? | Introduce product |
| 3 | Week 3 | [X] businesses have already signed up for verification | Social proof |
| 4 | Week 4 | Verification opens soon. Are you in? | Scarcity + final push |

## Email Strategy — Cold (New Prospects)

| # | Day | Subject | Angle |
|---|-----|---------|-------|
| 1 | 0 | Is someone copying [Business Name] online? | Problem alert |
| 2 | 3 | How much are fake pages costing Philippine businesses? | Agitate |
| 3 | 7 | The .ph registry just launched something for businesses like yours | Solution |
| 4 | 12 | [X] businesses in [industry] just got verified | Social proof |
| 5 | 18 | Last chance: first batch of verifications closing soon | Final push |

---

## Target Audiences

**Primary:** Business owners in the Philippines, 25-55, with online presence
**Priority industries:** Resorts/hotels, restaurants/F&B, online sellers, clinics, influencer-led brands

**Why these:** Highest impersonation risk. Clearest pain. Most likely to convert fast.

---

## Psychology Behind the Campaign

| Principle | How Used |
|-----------|----------|
| Loss Aversion | "Scammers are profiting from your name" — the fear of losing drives action more than the promise of gaining |
| Zero-Price Effect | "It's free" — psychologically removes all friction |
| Authority Bias | "From the official .ph domain registry" — credibility no one else can claim |
| Mimetic Desire | "Your competitors are getting verified" — people want what others want |
| Zeigarnik Effect | Email 1 deliberately has no CTA — it opens a loop that Email 2 closes |
| Social Proof | Live signup counter on landing page and in emails |
| Scarcity | "First batch" framing creates urgency without being dishonest |

---

## What "Verified" Is NOT (Do Not Imply)

- Government endorsement
- Absolute fraud protection
- Quality assurance of products or services

---

## Success Targets (8 Weeks)

| Metric | Target |
|--------|--------|
| Total signups | 1,000–3,000 |
| Email open rate (warm) | 30%+ |
| Landing page conversion | 15–25% |
| FB Ads cost per signup | < $2 |
| Organic FB engagement | 3%+ |

---

## Full Strategy Document

For complete copy (email bodies, ad copy variations, organic post templates, cold email sequence):
→ [verified-marketing-strategy.md](verified-marketing-strategy.md)
